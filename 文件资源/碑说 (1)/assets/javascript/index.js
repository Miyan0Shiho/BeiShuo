...
const app = createApp({
      ...
      handler(value) {
        this.$nextTick(() => {
          ...
          if (value === "app") {
            ...
            // 聚焦到iframe时，键盘左右切换不生效
            const iframeWeb = this.isHtml
              ? document.getElementById("iframe")
              : document.getElementById("vueIframeWeb");
            iframeWeb.addEventListener("load", () => {
              iframeDoc =
                iframeWeb.contentDocument || iframeWeb.contentWindow.document;
  ...
  methods: {
    ...
    communicationFun() {
      this.iframeMessage = useEventListener(
        window,
        "message",
        async (event) => {
          // 处理消息 判断data值是页面id时处理切换页面
          if (event.source != window) {
            const currentPage = this.pageList.find(
              (item) => item.pageUuid == event.data
            );
            ...
            this.currentIndex =
              this.pageList.findIndex(
                (item) => item.pageUuid == this.pageUuid
              ) || 0;
          }
        },
        true
    ...
    loadHtmlIframe() {
      ...
      if (htmlIframe) {
        const navIframe =
          htmlIframe.getElementById("navigation") ||
          htmlIframe.getElementById("contentFrame");
        this.$nextTick(() => {
          ...
          if (navIframe) {
            ...
            navIframe.addEventListener("load", () => {
              const navIframeDoc =
                navIframe.contentDocument || navIframe.contentWindow.document;
              this.childIframe = useEventListener(
                navIframeDoc,
                "keydown",
          ...
          var homeMenu = htmlIframe.querySelector(
            `[data-uuid="${this.pageUuid}"]`
          );
    ...
    handleChangePage(node) {
      ...
      this.currentIndex =
        this.pageList.findIndex((item) => item.pageUuid == node.uuid) || 0;
      this.pageUuid = node.uuid;
    },
    changeResolution() {
      if (this.pageType === "app") {
        const contentApp = document.querySelector(
        ...
        const scale = Math.min(
          contentApp.offsetWidth / Number(this.previewSizeW),
          contentApp.offsetHeight / Number(this.previewSizeH)
        );
        borderDom.style.transform = `scale(${scale})`;
      } else {
        if (this.appName) {
          const contentApp = document.querySelector(".page-content");
          let borderDom = this.isHtml
            ? document.getElementById("iframe")
            : document.getElementById("vueIframeWeb");
          const scale = Math.min(
            contentApp.offsetWidth / Number(this.previewSizeW),
    ...
    vueIfameloaded() {
      this.$nextTick(() => {
        const iframeId =
          this.pageType === "app" ? "#vueIframeApp" : "#vueIframeWeb";
        const iframeApp = document.querySelector(iframeId);
